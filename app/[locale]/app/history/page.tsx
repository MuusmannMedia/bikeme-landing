...
export default async function HistoryPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ notice?: string }> }) {
  ...
  return (
    <>
      ...
      <AppPanel>
        {result.value.length ? <div className="bike-app-history-table-wrap"><table className="bike-app-table"><thead><tr><th>{t("history.details")}</th><th>{t("rides.distance")}</th><th>{t("history.duration")}</th><th>{t("history.elevation")}</th><th><span className="sr-only">{t("common.view")}</span></th></tr></thead><tbody>{result.value.map((ride) => <tr key={ride.id}><td><strong>{ride.title}</strong><small>{formatDate(locale, ride.startedAt)} · {ride.discipline}</small></td><td>{formatDistance(locale, ride.distanceKm, viewer.profile.unitSystem)}</td><td>{formatDuration(ride.durationMinutes)}</td><td>{formatElevation(locale, ride.elevationGain, viewer.profile.unitSystem)}</td><td><Link className="bike-app-button bike-app-button-secondary bike-app-button-small" href={`/${locale}/app/history/${ride.id}`}>{t("common.view")}</Link></td></tr>)}</tbody></table></div> : <AppEmpty>{t("common.empty")}</AppEmpty>}
      </AppPanel>
    </>
  );
}
