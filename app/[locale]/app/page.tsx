...
export default async function ProtectedPage({ params, searchParams }: ProtectedPageProps) {
  ...
  const [ridesResult, connectionsResult, invitesResult, interestResult, historyResult] = await Promise.allSettled([
    ...
    viewer.access.hasPro ? listRideHistory(client, viewer.userId, overviewRecentRideLimit) : Promise.resolve([])
  ]);
  ...
  const pendingInvites = selectPendingReceivedInvites(invites);
  const pendingInterest = selectPendingReceivedInterest(interest);
  const acceptedConnections = selectAcceptedConnections(connections);
  const recentValue = viewer.access.hasPro ? history.length : t("common.pro");
  const invitationsHref = `/${locale}/app/requests?view=invites`;
  const interestHref = `/${locale}/app/requests?view=interest`;
  const connectionsHref = `/${locale}/app/riders?view=connected`;
  const historyHref = `/${locale}/app/history?view=recent`;
  const metricLabel = (label: string, value: string | number) => t("overview.viewMetric")
    .replace("{label}", label)
  ...
  return (
    <>
      <AppPageHeader eyebrow={t("shell.eyebrow")} title={t("overview.title")} intro={t("overview.intro")} />
      <AppNotice locale={locale} code={hasLoadError ? "data" : notice} />
      <div className="bike-app-grid" data-columns="4">
        <Link aria-label={metricLabel(t("overview.invitations"), pendingInvites.length)} className="bike-app-stat" href={invitationsHref}><span>{t("overview.invitations")}</span><strong>{pendingInvites.length}</strong></Link>
        <Link aria-label={metricLabel(t("overview.interest"), pendingInterest.length)} className="bike-app-stat" href={interestHref}><span>{t("overview.interest")}</span><strong>{pendingInterest.length}</strong></Link>
        <Link aria-label={metricLabel(t("overview.connections"), acceptedConnections.length)} className="bike-app-stat" href={connectionsHref}><span>{t("overview.connections")}</span><strong>{acceptedConnections.length}</strong></Link>
        <Link aria-label={metricLabel(t("overview.recent"), recentValue)} className="bike-app-stat" href={historyHref}><span>{t("overview.recent")}</span><strong>{recentValue}</strong></Link>
      </div>
      <div className="bike-app-grid bike-app-overview-grid" data-columns="2">
        ...
        <AppPanel title={t("overview.invitations")} action={<Link href={invitationsHref}>{t("common.view")}</Link>}>
          {pendingInvites.length ? <ul className="bike-app-list">{pendingInvites.slice(0, 4).map((invite) => (
            <li key={`${invite.rideId}-${invite.inviteeId}`} className="bike-app-list-item">
              ...
              <Link className="bike-app-button bike-app-button-secondary bike-app-button-small" href={invitationsHref}>{t("common.view")}</Link>
            </li>
          ))}</ul> : <AppEmpty>{t("common.empty")}</AppEmpty>}
        </AppPanel>
        <AppPanel title={t("overview.interest")} action={<Link href={interestHref}>{t("common.view")}</Link>}>
        ...
        <AppPanel title={t("overview.recent")} action={<Link href={historyHref}>{t("common.view")}</Link>}>
